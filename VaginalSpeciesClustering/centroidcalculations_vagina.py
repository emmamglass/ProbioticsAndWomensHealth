...
# Load the dataset
file_path = "data_to_plot_vagina.csv"
...
classification_order = ["Vaginal Commensal", "Vaginal Probiotic"]
...
# create palette
palette = {
    "Vaginal Commensal": "#167dcc",
    "Vaginal Probiotic": "#3c7f4d"
}

# Create the scatter plot
plt.figure(figsize=(10, 6))
scatter = sns.scatterplot(data=df, x="Component 1", y="Component 2", hue=classification_col, palette=palette, alpha=0.4, edgecolor='black', s=20)
...
for _, row in centroids.iterrows():
    ...
    # Scatter centroid
    plt.scatter(centroid_x, centroid_y, color=color, marker='^', s=200, edgecolors='black', label=f"{class_name} Centroid")
    ...
    if class_name in cov_matrices:
        ...
        ellipse = Ellipse((centroid_x, centroid_y), width, height, angle, edgecolor='black', facecolor='none', linestyle="-", linewidth=3)
        ax.add_patch(ellipse)

        ellipse = Ellipse((centroid_x, centroid_y), width, height, angle, edgecolor=color, facecolor='none', linestyle="-", linewidth=2)
        ax.add_patch(ellipse)
