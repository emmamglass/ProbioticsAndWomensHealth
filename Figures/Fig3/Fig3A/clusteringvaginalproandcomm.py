...
def cluster(clustertype='pca', downsample=500):
	...
	for file in glob.glob('*.sbml.csv'):
		with open('vaginal_probiotic.txt') as f:
			if str(file) in f.read():
				classification = str('Vaginal Probiotic')
			else:	
				with open('vagina_commensals_flux_list.txt') as f:
					if str(file) in f.read():
						classification = str('Vaginal Commensal')
					else:
						continue
		print(file)
		i+=1
	...
	data_to_plot.to_csv('data_to_plot_vagina.csv')
	return data_to_plot
