...
This folder is named for its manuscript panel target: `Fig2G` corresponds to Figure 2, panel G.
...
- Flux sampling CSVs are stored outside this folder in `../FluxSamplingFiles/HumanAssociated/` (relative to `Fig2G/`).
...
**Important path note**:
- `Clustering.py` currently uses `glob.glob('*.sbml.csv')`, so by default it only finds CSVs in the current working directory.
- Since the CSVs are in `../FluxSamplingFiles/HumanAssociated/`, either:
  - run the script from that flux folder while keeping list files accessible, or
  - update `Clustering.py` to glob `../FluxSamplingFiles/HumanAssociated/*.sbml.csv`, or
  - copy/symlink the needed CSVs into `Fig2G/`.
...
1. **Generate flux data**: Run gap-split flux sampling on probiotic SBML models to produce `.sbml.csv` files in `FluxSamplingFiles/HumanAssociated/`
